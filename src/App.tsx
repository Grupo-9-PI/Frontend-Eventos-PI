...
function CrearEvento() {
  ...
  const [datos, setDatos] = useState<FormEvento>(formEventoInicial); const [tareas, setTareas] = useState<BorradorTarea[]>(() => [ { ...borradorInicial(), titulo: 'Reservar salón', categoria: 'salon', prioridad: 'alta', estimacion: '2' } ]); const [nuevo, setNuevo] = useState<BorradorTarea>(borradorInicial); const [error, setError] = useState('');
  ...
  return <div><Encabezado eyebrow='Nuevo plan' titulo='Crear evento' /><form onSubmit={submit} className='card form-card' noValidate><section className='form-section'><h2 className='form-section-title'>Identidad del evento</h2><div className='form-grid'><div className='field full'><label>Nombre *</label><input value={datos.nombre} onChange={(e) => setCampo('nombre', e.target.value)} /></div><div className='field'><label>Tipo</label><input value={datos.tipo} onChange={(e) => setCampo('tipo', e.target.value)} /></div><div className='field'><label>Lugar</label><input value={datos.lugar} onChange={(e) => setCampo('lugar', e.target.value)} /></div><div className='field'><label>Inicio *</label><input type='date' value={datos.fechaInicio} onChange={(e) => setCampo('fechaInicio', e.target.value)} /></div><div className='field'><label>Fin *</label><input type='date' value={datos.fechaFin} onChange={(e) => setCampo('fechaFin', e.target.value)} /></div><div className='field'><label>Hora *</label><input type='time' value={datos.horaEvento} onChange={(e) => setCampo('horaEvento', e.target.value)} /></div><div className='field'><label>Duración (h) *</label><input type='number' min='0.5' step='0.5' value={datos.duracion} onChange={(e) => setCampo('duracion', e.target.value)} /></div><div className='field'><label>Límite diario (h) *</label><input type='number' min='1' step='0.5' value={datos.capacidadDiaria} onChange={(e) => setCampo('capacidadDiaria', e.target.value)} /></div><div className='field full'><label>Notas</label><textarea value={datos.notas} onChange={(e) => setCampo('notas', e.target.value)} /></div></div></section><section className='form-section'><h2 className='form-section-title'>Plan inicial</h2><div className='task-builder'>{tareas.map((t) => <div className='task-draft' key={t.id}><div className='task-draft-info'>{t.titulo}</div><button type='button' className='button button-small button-danger button-icon' onClick={() => setTareas((ts) => ts.filter((x) => x.id !== t.id))}><Trash2 size={13} /></button></div>)}<div className='card card-pad'><div className='task-builder-row'><div className='task-builder-item'><label>Nueva gestión</label><input value={nuevo.titulo} onChange={(e) => setNuevo({ ...nuevo, titulo: e.target.value })} /></div><div className='task-builder-item'><label>Est. (h)</label><input type='number' min='0.25' step='0.25' value={nuevo.estimacion} onChange={(e) => setNuevo({ ...nuevo, estimacion: e.target.value })} /></div></div><div className='task-builder-row compact' style={{ marginTop: 11 }}><div className='task-builder-item'><label>Plazo</label><input type='date' value={nuevo.fechaLimite} max={datos.fechaInicio} onChange={(e) => setNuevo({ ...nuevo, fechaLimite: e.target.value })} /></div><div className='task-builder-item'><label>Hora</label><input type='time' value={nuevo.horaLimite} onChange={(e) => setNuevo({ ...nuevo, horaLimite: e.target.value })} /></div><div className='task-builder-item'><button type='button' className='button button-secondary' onClick={añadirTarea}><Plus size={14} /> Agregar</button></div></div></div></div></section>{error && <div className='error-box'>{error}</div>}<div className='form-actions'><Link href='/eventos' className='button button-ghost'>Cancelar</Link><button className='button button-primary' type='submit'><Check size={15} /> Crear evento y plan</button></div></form></div>;
}
...
function TaskEditorDialog({ evento, tarea, onClose, onSave }: { evento: Evento; tarea: Subtarea; onClose: () => void; onSave: (t: Subtarea) => void }) {
...
function ReprogramarDialog({ evento, tarea, onClose, onSave }: { evento: Evento; tarea: Subtarea; onClose: () => void; onSave: (fecha: string, hora: string) => void }) {
  ...
  const confirmar = (e: FormEvent) => { e.preventDefault(); const problema = validarTarea(evento, { ...tarea, fechaLimite: fecha, horaLimite: hora }, evento.subtareas.filter((t) => t.id !== tarea.id)); if (problema) { setError(problema); return; } onSave(fecha, hora); };
